...
public class SpellResultCalculator {
    ...
    public static SpellResult Result(Item... runes) {
        ...
        for (Map.Entry<Integer, ArrayList<SpellEffect>> entry : mappedEffects.entrySet()) {
            ...
            if (entry.getValue().size() > 2 || (entry.getValue().size() == 2 && entry.getValue().get(0).getType().equals(entry.getValue().get(1).getType()))) {
        ...
        for (Map.Entry<Integer, ArrayList<SpellEffect>> entry : mappedEffects.entrySet()) {
            ...
            OUTER_LOOP : for (int a = 0; a < entry.getValue().size(); a++) {
                for (int b = 0; b < entry.getValue().size(); b++) {
                    ...
                    if (containsKey(entry.getValue().get(a).getLevelUps(), entry.getValue().get(b))) {
                        SpellEffect ef = get(entry.getValue().get(a).getLevelUps(), entry.getValue().get(b));
                        if (resultVal.indexOf(get(entry.getValue().get(a).getLevelUps(), entry.getValue().get(b))) == -1)
                            resultVal.add(get(entry.getValue().get(a).getLevelUps(), entry.getValue().get(b)));
                        continue OUTER_LOOP;
                    }
